...
import { getAllPages, getNavigation, getPagesByParentId, getPagesBySlug } from '@/payload/payloadClient';
import { NavigationData, PageData, PayloadMedia, TileSheetBlock } from '@/payload/payload-types';
...
export async function getStaticProps({ params, draftMode }: { params: RouteParams, draftMode: boolean }) {
  ...
  if (currentPage.content) {
    ...
    if (parentSheetIndex > -1) {
      const childPages = await getPagesByParentId(currentPage.id);
